...
class TextureInpaintDataset(BaseDataset):
    ...
    def get_paths(self, opt):
        ...
        image_paths = [i for i in np.genfromtxt(os.path.join(root, opt.dataset_name, opt.phase+'.flist'), dtype=str, encoding='utf-8')]
        if opt.mask_type >= 3:
    ...
    def load_mask(self, index):
        ...
        if mask_type == 6:
            ...
            for _ in range(10):
                t1, l1 = random.randint(4*self.h//8, 5*self.h//8), random.randint(2*self.w//7, 3*self.w//7)
                t2, l2 = random.randint(5*self.h//8, 6*self.h//8), random.randint(4*self.w//7, 5*self.w//7)
                m[t1:t2, l1:l2] = 255
            return m
